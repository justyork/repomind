...
describe('check', () => {
  ...
  it('fails on broken related slug', () => {
    const repo = makeTempDir();
    runInit({ cwd: repo });
    const docPath = path.join(repo, '.project-knowledge/adr/use-plain-markdown.md');
...
describe('init', () => {
  it('scaffolds example docs including combat-system', () => {
    const repo = makeTempDir();
    runInit({ cwd: repo });
    expect(fs.existsSync(path.join(repo, '.project-knowledge/specs/combat-system.md'))).toBe(true);
    expect(fs.existsSync(path.join(repo, '.project-knowledge/glossary/mcp.md'))).toBe(true);
  });

  it('is idempotent and does not clobber user docs', () => {
    const repo = makeTempDir();
    runInit({ cwd: repo });
    const docPath = path.join(repo, '.project-knowledge/glossary/mcp.md');
