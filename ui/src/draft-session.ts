...
import { escapeHtml, type PageShellRefs } from './page-shell.js';
...
export function bindDraftSession(
  ...
  let docEscapeHandler: ((event: KeyboardEvent) => void) | null = null;
  ...
  container.querySelector<HTMLButtonElement>('#ed-close')?.addEventListener('click', onClose);
  ...
  docEscapeHandler = (event: KeyboardEvent) => {
    if (event.key === 'Escape' && modal.classList.contains('hidden') && viewMdModal.classList.contains('hidden')) {
      onClose();
    }
  };
...
export function renderEditorActionsHtml(focusMode: boolean, options?: { showDiscard?: boolean }): string {
  const showDiscard = options?.showDiscard ?? true;
  return `
    <div class="editor-action-bar">
      <span class="editing-badge" aria-hidden="true">Editing</span>
      <span id="ed-save-status" class="save-status" aria-live="polite"></span>
      <div class="editor-action-spacer" aria-hidden="true"></div>
      <div class="editor-action-primary">
        <div class="publish-split">
          <button type="button" id="ed-publish" class="btn-primary publish-main">Publish</button>
          <button type="button" id="ed-publish-menu-btn" class="btn-primary publish-menu-trigger" aria-haspopup="true" aria-expanded="false" title="More publish options">▾</button>
      ...
      <div class="editor-action-secondary">
        <button type="button" id="ed-view-md" class="btn-ghost btn-sm">Markdown</button>
        <button type="button" id="toggle-focus" class="btn-ghost btn-sm" aria-pressed="${focusMode}">
          ${focusMode ? 'Show info' : 'Hide info'}
        </button>
        <button type="button" id="ed-close" class="btn-ghost btn-sm">Close</button>
        ${showDiscard ? '<button type="button" id="ed-discard" class="btn-ghost btn-sm btn-danger-text">Discard</button>' : ''}
      </div>
    </div>
  `;
}
