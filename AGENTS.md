...
- Reader and editor content should use the full available workspace width (not a fixed ~72ch column).
...
- npm package `@justyork/repo-mind` on registry (scoped due to npm `repomind` name conflict); CLI `repo-mind`. Current on `main`: **0.8.1**. Agent write: `create_draft` MCP and `repo-mind publish --pr` use SQLite drafts (not git worktree); gated on ab-demo kill-switch P2 (`REPOMIND_AGENT_WRITE=1` override for local dev).
